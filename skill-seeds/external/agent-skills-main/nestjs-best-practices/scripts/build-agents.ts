#!/usr/bin/env npm exec --no -- ts-node

/**
 * Build script for generating AGENTS.md from individual rule files
 *
 * Usage: npm exec --no -- ts-node scripts/build-agents.ts
 *
 * This script:
...
function resolveInsidePackage(relativePath: string, mustExist = true): string {
  const candidate = path.resolve(packageRoot, relativePath);
  const relative = path.relative(packageRoot, candidate);
  if (!relative || relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
    throw new Error(`Path escapes skill package: ${relativePath}`);
  }
  if (mustExist) {
    const realCandidate = fs.realpathSync(candidate);
    const realRelative = path.relative(packageRoot, realCandidate);
    if (realRelative === '..' || realRelative.startsWith(`..${path.sep}`) || path.isAbsolute(realRelative)) {
      throw new Error(`Path resolves outside skill package: ${relativePath}`);
    }
    return realCandidate;
  }
  if (fs.existsSync(candidate) && fs.lstatSync(candidate).isSymbolicLink()) {
    throw new Error(`Refusing to overwrite symlink: ${candidate}`);
  }
  return candidate;
}
...
function readMetadata(): any {
  const metadataPath = resolveInsidePackage('metadata.json');
  return JSON.parse(fs.readFileSync(metadataPath, 'utf-8'));
}

function readRules(): Rule[] {
  const rulesDir = resolveInsidePackage('rules');
  ...
  for (const file of files) {
    const filePath = resolveInsidePackage(path.join('rules', file));
...
function main() {
  ...
  const outputPath = resolveInsidePackage('AGENTS.md', false);
