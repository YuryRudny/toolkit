...
2. Run sync: `npm exec --no -- cap sync`
...
**Common causes**:
1. **iOS ATS blocking HTTP**: keep production traffic on HTTPS. For a local
development host only, use a domain-scoped exception in a development-specific
`Info.plist`; never enable a global release exception:
```xml
<key>NSAppTransportSecurity</key>
<dict>
    <key>NSExceptionDomains</key>
    <dict>
        <key>dev.example.internal</key>
        <dict>
...
- [ ] If dependency corruption is evidenced, ask before deleting local artifacts and use the repository package manager's documented clean/frozen install workflow
