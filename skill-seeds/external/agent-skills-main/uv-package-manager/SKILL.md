...
```bash
# Use the project's approved package manager and pin the reviewed version.
# Do not pipe downloaded scripts into a shell. If a standalone artifact is
# required, download it separately and verify its published checksum/signature.
python -m pip install "uv==<reviewed-version>"

# On managed macOS, use the approved Homebrew formula after reviewing its source.
brew install uv
