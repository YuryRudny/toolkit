...
Session state and login utilities are **infrastructure**, not business logic.
Keep browser authentication on a server-managed session with `HttpOnly`,
`Secure`, `SameSite` cookies. Do not persist bearer/session tokens in
...
export const apiClient = axios.create({
  ...
  // Browser auth uses a server-issued HttpOnly cookie. The API must enforce
  // SameSite/CSRF protections and an explicit credentialed CORS origin.
