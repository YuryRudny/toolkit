 ...
 */
function resolveProjectOutput(relativePath: string): string {
  ...
  if (parentRelative === ".." || parentRelative.startsWith(`..${path.sep}`) || path.isAbsolute(parentRelative)) {
    throw new Error(`Output directory resolves outside project root: ${parent}`);
  }
  if (fs.existsSync(outputPath) && fs.lstatSync(outputPath).isSymbolicLink()) {
    throw new Error(`Refusing to overwrite symlink: ${outputPath}`);
  }
  return outputPath;
}

function emitFile(list: string[]): void {
  const outPath = resolveProjectOutput("src/app/styles/daisy-safelist.css");
...
`;
  ...
  try {
    fs.writeFileSync(tempPath, fileContents, { encoding: "utf8", flag: "wx", mode: 0o600 });
