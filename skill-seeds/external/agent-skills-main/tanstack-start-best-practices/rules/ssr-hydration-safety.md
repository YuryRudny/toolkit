...
// Do not hide mismatches or render untrusted HTML. Normalize rich content on
// the server with a reviewed allowlist sanitizer and pass the exact same safe
// representation to server and client.
function UserContent({ safeText }: { safeText: string }) {
  return (
    <div>{safeText}</div>
  )
}
