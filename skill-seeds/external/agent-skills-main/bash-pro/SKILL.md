...
- End option parsing with `--`; before deletion canonicalize the path and require it to remain inside an explicit allowlisted working directory
...
- Never delete a raw user-supplied path. Resolve it canonically, reject root/empty/parent paths, require containment in an allowlisted temporary workspace, then use `--`
