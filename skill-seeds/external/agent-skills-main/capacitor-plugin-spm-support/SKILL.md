...
Then `cd` into the example or test app directory that contains `capacitor.config.*`, run `npm exec --no -- cap sync` (or the repository's equivalent runner) there, and build that same app.
