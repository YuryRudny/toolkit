...
npm exec --no -- capsec scan

# Scan specific path
npm exec --no -- capsec scan ./my-app

# CI mode (exit code 1 on high/critical issues)
npm exec --no -- capsec scan --ci
```
...
# JSON report
npm exec --no -- capsec scan --output json --output-file report.json

# HTML report
npm exec --no -- capsec scan --output html --output-file security-report.html
```
...
npm exec --no -- capsec scan --severity high

# Specific categories
npm exec --no -- capsec scan --categories secrets,network,storage

# Exclude test files
npm exec --no -- capsec scan --exclude "**/test/**,**/*.spec.ts"
```
  ...
  security:
    ...
    steps:
      ...
      - name: Run Capsec Security Scan
        run: npm exec --no -- capsec scan --ci --output json --output-file security-report.json
...
- [ ] Run `npm exec --no -- capsec scan --severity high`
