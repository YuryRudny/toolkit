...
class ArkUISkillTestRunner {
  ...
  saveReport(report: EvaluationReport, outputPath: string): void {
    const allowedDir = path.join(__dirname, 'results');
    fs.mkdirSync(allowedDir, { recursive: true });
    const realAllowedDir = fs.realpathSync(allowedDir);
    const resolvedOutput = path.resolve(outputPath);
    const relative = path.relative(realAllowedDir, resolvedOutput);
    if (!relative || relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
      throw new Error(`Report output escapes tests/results: ${outputPath}`);
    }
    if (path.extname(resolvedOutput) !== '.json') {
      throw new Error('Evaluation report output must use .json extension');
    }
    if (fs.existsSync(resolvedOutput) && fs.lstatSync(resolvedOutput).isSymbolicLink()) {
      throw new Error(`Refusing to overwrite symlink: ${resolvedOutput}`);
    }

    // Save JSON report
    fs.writeFileSync(resolvedOutput, JSON.stringify(report, null, 2), { encoding: 'utf8', flag: 'wx', mode: 0o600 });

    // Save human-readable report
    const textReport = this.formatTextReport(report);
    const textPath = resolvedOutput.replace(/\.json$/, '.txt');
    fs.writeFileSync(textPath, textReport, { encoding: 'utf8', flag: 'wx', mode: 0o600 });
