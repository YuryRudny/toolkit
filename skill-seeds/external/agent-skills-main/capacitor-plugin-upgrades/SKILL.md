...
3. Run `npm install`.
4. Run `npm exec --no -- cap sync` from the example or test app directory that contains `capacitor.config.*`, or rebuild that app with the repository's documented command.
...
- `npm run build`
- `npm test`
- `npm exec --no -- cap sync` from the example/test app directory
