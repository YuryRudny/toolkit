...
jobs:
  ...
  security:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-node@v4
      - run: npm exec --no -- capsec scan --ci
  ...
  build-android:
    ...
    steps:
      ...
      - name: Sync Capacitor
        run: npm exec --no -- cap sync android
  ...
  deploy-capgo:
    ...
    steps:
      ...
      - name: Deploy to Capgo
        run: npm exec --no -- @capgo/cli upload
        env:
...
security:
  ...
  image: node:20
  script:
    - npm exec --no -- capsec scan --ci --output json --output-file security.json
  artifacts:
    reports:
...
deploy-capgo:
  ...
  needs: [build-web]
  script:
    - npm exec --no -- @capgo/cli upload --channel production
  only:
    - main
  environment:
    name: production
  ...
  release:
    ...
    steps:
      ...
      - name: Release
        ...
        run: npm exec --no -- semantic-release
```
